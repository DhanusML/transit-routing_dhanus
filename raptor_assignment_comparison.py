...
def get_available_options_ser(od_mat: np.ndarray) -> list:
    ...
    """
    pareto_journeys = []
    for origin, destination, d_time in tqdm(od_mat):
        time = _gen_random_date_time()
        output = raptor_dhanus(origin, destination, d_time, MAX_TRANSFER,
...
def get_optimal_choices(od_mat, beta, op='ser'):
    """
    ...
    if op == 'ser':
        ...
        list_of_pareto_journeys = get_available_options_ser(od_mat)
        time_taken_rap = time.time() - start_time
        ...
        raise Exception(f"invalid option {op}")
    ...
    time_taken_choice = time.time() - start_time

    return time_taken_rap, time_taken_choice, selected_journeys
...
def get_segment_occupancy(journeys):
    ...
    return time_taken, occupancy_dict


def main():
    print("\nrunning in serial mode")
    t_r, t_c, selected_journeys = get_optimal_choices(od_mat, beta, 'ser')
    t_seg, occupancy_dict = get_segment_occupancy(selected_journeys)

    print("\nserial outputs")
    print(f'num o-d pairs: {NUM}')
    print(f'time for raptor: {t_r}')
    print(f'time for choice: {t_c}')
    print(f'time for occupancy dict generation: {t_seg}')
    print('num journeys', len(selected_journeys))

    print(f'\nrunning in parallel mode with {CORES} cores')
    t_r, t_c, selected_journeys = get_optimal_choices(od_mat, beta, 'par')
    t_seg, occupancy_dict = get_segment_occupancy(selected_journeys)
...
if __name__ == "__main__":
    ...
    NUM = 100
    beta = [-0.1, -2]
    od_mat = generate_od_matrix_with_time(NUM)
    main()
